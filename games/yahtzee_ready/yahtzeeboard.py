...
class YahtzeeBoard:
    ...
    def categoryListener(self, row):
        ...
        # TODO: 구현
        if cur_player.allUpperUsed() == True:
            ...
            if cur_player.getUpperScore() >= 63:
                ...
                cur_player.setScore(35,self.UPPERBONUS)
            else:
                self.fields[self.UPPERBONUS][self.player]['text'] = 0
                cur_player.setScore(0,self.UPPERBONUS)
        ...
        # TODO: 구현
        if cur_player.allLowerUsed() == True:
            self.fields[self.LOWERTOTAL][self.player]['text'] = cur_player.getLowerScore()

        # UPPER category와 LOWER category가 전부 사용되었으면 
        # -> UI의 TOTAL 에 내용 채우기.
        # TODO: 구현
        if cur_player.allLowerUsed() == True and cur_player.allUpperUsed() == True:
            self.fields[self.TOTAL][self.player]['text'] = cur_player.getLowerScore() + cur_player.getUpperScore()
