...
def VaccinationCenter():
    ...
    # response = HospList[0].childNodes #root
    ...
    CenterList = CenterDoc.childNodes
    results = CenterList[0].childNodes
    data = results[1].childNodes
    #item = data[0].childNodes

    for item in data:
        centers = item.childNodes
...
#  ---------------------------------------------------------------------
HosInfoButton = Button(SelectButtonFrame,text='병원 정보',height=5,width=20, command = VaccinationCenter())
