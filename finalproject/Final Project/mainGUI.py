...
def InitScreen():
    ...
    titleImage = PhotoImage(file=r"image\logo.png")
    titleText = Label(TopFrame, image=titleImage, font=fontTitle, relief='ridge')
    titleText.grid(row=0, column=0)

    emailImage = PhotoImage(file=r"image\email.png")  # 이메일 이미지 추가
    ...
    redirecImage = PhotoImage(file=r"image\redirec.png")
    ...
    # 5번째 증상 및 대처
    ##########################################################################
    symptomImage = PhotoImage(file=r"image\symptom.png") # 증상 이미지 추가
    copImage = PhotoImage(file=r"image\cope.png") # 대처 이미지
